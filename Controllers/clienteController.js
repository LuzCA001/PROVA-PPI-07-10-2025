  ...
  async gravar(req, res) {
    ...
    try {
      ...
      if (error.code === 'ER_DUP_ENTRY') {
        return res.status(409).json({ status: false, message: "Este CPF já está cadastrado." });
      }
  ...
  async atualizar(req, res) {
    ...
    try {
      ...
      if (error.code === 'ER_NO_REFERENCED_ROW_2') {
        return res.status(400).json({ status: false, message: "Um ou mais livros não foram encontrados para associação." });
      }
